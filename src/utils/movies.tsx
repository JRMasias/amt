import movie1 from "../assets/movie_1.jpg";
import movie_1_mobile from "../assets/movie_1_mobile.png";
import showcaseImage from "../assets/showcase_1.jpg";
import movie2 from "../assets/movie_2.jpg";
import movie_2_mobile from "../assets/movie_2_mobile.png";
import showcaseImage2 from "../assets/showcase_2.jpg";
import movie3 from "../assets/movie_3.jpg";
import movie_3_mobile from "../assets/movie_3_mobile.png";
import showcaseImage3 from "../assets/showcase_3.jpg";
import movie4 from "../assets/movie_4.jpg";
import movie_4_mobile from "../assets/movie_1_mobile.png";
import showcaseImage4 from "../assets/showcase_4.jpg";
import movie5 from "../assets/movie_5.jpg";
import movie_5_mobile from "../assets/movie_1_mobile.png";
import showcaseImage5 from "../assets/showcase_5.jpg";
import movie6 from "../assets/movie_6.jpg";
import movie_6_mobile from "../assets/movie_1_mobile.png";
import movie7 from "../assets/movie_7.jpg";
import movie_7_mobile from "../assets/movie_1_mobile.png";
import movie8 from "../assets/movie_8.jpg";
import movie_8_mobile from "../assets/movie_1_mobile.png";
import movie9 from "../assets/movie_9.jpg";
import movie_9_mobile from "../assets/movie_1_mobile.png";
import movie10 from "../assets/movie_10.jpg";
import movie_10_mobile from "../assets/movie_1_mobile.png";
import movie11 from "../assets/movie_11.jpg";
import movie_11_mobile from "../assets/movie_1_mobile.png";

export const movies = [
     {
          id: 1,
          title: "Super Action Movie",
          description: "An action-packed adventure that will keep you on the edge of your seat.",
          image: movie1,
          imageMobile: movie_1_mobile,
          rating: 4.5,
     },
     {
          id: 2,
          title: "Afraid",
          description: "A psychological thriller that will keep you guessing until the very end.",
          image: movie2,
          imageMobile: movie_2_mobile,
          rating: 4.8,
     },
     {
          id: 3,
          title: "Serious Film",
          description: "A thought-provoking drama that will leave you reflecting long after the credits roll.",
          image: movie3,
          imageMobile: movie_3_mobile,
          rating: 4.3,
     },
     {
          id: 4,
          title: "The Princess Elf",
          description: "A magical fantasy tale that will transport you to a world of wonder and enchantment.",
          image: movie4,
          imageMobile: movie_4_mobile,
          showcaseImage: showcaseImage,
          rating: 4.9,
     },
     {
          id: 5,
          title: "Dark of Space",
          description: "A sci-fi epic that will take you on a thrilling journey through the cosmos.",
          image: movie5,
          imageMobile: movie_5_mobile,
          showcaseImage: showcaseImage2,
          rating: 4.1,
     },
     {
          id: 6,
          title: "Super Hero Movie: The Moviefilm",
          description: "A superhero adventure that will keep you on the edge of your seat.",
          image: movie6,
          imageMobile: movie_6_mobile,
          showcaseImage: showcaseImage3,
          rating: 3.9,
     },
     {
          id: 7,
          title: "My Funny Tragedy",
          description: "A comedic tragedy that will make you laugh and cry at the same time.",
          image: movie7,
          imageMobile: movie_7_mobile,
          showcaseImage: showcaseImage5,
          rating: 3.8,
     },
     {
          id: 8,
          title: "New Love Again 2: Electric Boogaloo",
          description: "A romantic comedy that will warm your heart and make you believe in love again.",
          image: movie8,
          imageMobile: movie_8_mobile,
          showcaseImage: showcaseImage4,
          rating: 4.0,
     },
     {
          id: 9,
          title: "Action Fight IV",
          description: "An action-packed adventure that will keep you on the edge of your seat.",
          image: movie9,
          imageMobile: movie_9_mobile,
          rating: 4.0,
     },
     {
          id: 10,
          title: "History Movie",
          description: "A historical drama that will transport you back in time and immerse you in the past.",
          image: movie10,
          imageMobile: movie_10_mobile,
          rating: 2.7,
     },
     {
          id: 11,
          title: "Still Terrified",
          description: "A horror movie that will keep you on the edge of your seat and make you jump out of your skin.",
          image: movie11,
          imageMobile: movie_11_mobile,
          rating: 4.9,
     },

];